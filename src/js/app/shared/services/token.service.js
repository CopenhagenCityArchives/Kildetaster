    ...
    var tokenService = /*@ngInject*/ function tokenService($q, angularAuth0, $sessionStorage, $location) {
        ...
        return {
            ...
            getTokenFromCallBack: function(){
                ...
                angularAuth0Provider.parseHash({ hash: window.location.hash },function(err, authResult) {
                    if (err) {
                    ...
                    angularAuth0Provider.client.userInfo(authResult.accessToken, function(err, user) {
                        if(err){
